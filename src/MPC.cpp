  ...
  void operator()(ADvector &fg, const ADvector &vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      AD<double> f0 = coeffs[0] +
          coeffs[1] * CppAD::pow(x0, 1) +
          coeffs[2] * CppAD::pow(x0, 2) +
          coeffs[3] * CppAD::pow(x0, 3);

      AD<double> psides0 = CppAD::atan(coeffs[1] +
          coeffs[2] * 2.0 * CppAD::pow(x0, 1) +
          coeffs[3] * 3.0 * CppAD::pow(x0, 2));
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  const double k_steering_angle = 0.436332;

  // The upper and lower limits of delta are set to -25 and 25
  // degrees (values in radians).
  // NOTE: Feel free to change this to something else.
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -k_steering_angle;
    vars_upperbound[i] = k_steering_angle;
  }
  ...
  if (ok) {
    for (size_t i = 0; i < N; ++i) {
      mpc_x_vals.emplace_back(solution.x[x_start + i]);
      mpc_y_vals.emplace_back(solution.x[y_start + i]);
    }
    ...
    return {2.0 * (solution.x[delta_start] + k_steering_angle) / (2.0 * k_steering_angle) - 1.0,
            solution.x[a_start]};
  } else {
    return {0, 0};
  }
